...
async def evaluate_mood(profile_name: str, db_path: str | None = None) -> dict | None:
    ...
    # Mood scoring: the LLM backbone + per-profile corrections feed the shared shaping layer
    # (cascade → tenure-bias → cooldown → hysteresis → winner), then exit triggers may override.
    v2vm, v2bd, _v2dbg = await _generate_v2_votes(profile_name, enriched, moods)
